...
**Already compliant, no changes needed:** scroll-reveal pattern (fade + translate, fires once, disabled under reduced-motion, one reveal per section not per child); Bespoke's lightbox (Escape closes, arrows navigate, focus-trapped, focus returns to trigger on close, no autoplay); no cursor-follow/WebGL effects, no text-baked-into-images, no travel pictograms in motion anywhere in the codebase.

**Scoped exception added 2026-08-24:** the "no carousels/sliders/rotators, no marquee logo scroller" line above no longer holds site-wide. Andy explicitly asked for the Bespoke lock screen's partner-logo strip to be more visually interesting, and confirmed (after being asked directly, since it reverses a documented decision) that he specifically wants a sliding/rotating carousel there, not just a static resize. Built as a CSS-only continuous auto-scroll marquee in `pages/lock-screen.html` — pauses on hover, freezes to a static wrapped row under `prefers-reduced-motion`, logo list duplicated once (aria-hidden) for a seamless loop. This is scoped to that one element; every other page still has zero carousels/sliders/rotators as before.
